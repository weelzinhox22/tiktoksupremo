...
function VideoEditorPage() {
  ...
  const segmentOrderFor = (combination: EditorCombination) =>
    timelineOrders[combination.number] ?? segmentIdsForCombination(combination);
        ...
        <div className="relative flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold tracking-tight md:text-4xl">
              Juntar e embaralhar vídeos
            </h1>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground">
              Combine ganchos, corpos e CTAs em escala e baixe tudo em um único ZIP. Para cortes,
              textos, áudio e animações, use o Editor de vídeo separado.
            </p>
          </div>
            ...
            {recommendedCombinations.map((item, index) => (
              <button
                ...
                onClick={() => {
                  ...
                  document.getElementById("batch-imports")?.scrollIntoView({ behavior: "smooth" });
                }}
              >
      ...
      <section className="flex flex-col gap-4 rounded-2xl border border-primary/20 bg-primary/[0.055] p-5 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            O editor completo agora tem uma página própria, com textos, áudio, atalhos e animações.
          </p>
        </div>
        ...
        <div className="surface-card p-5 md:p-6">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            {[
              ...
              <article key={title} className="rounded-xl border border-border bg-secondary/20 p-4">
