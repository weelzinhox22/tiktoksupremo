...
import { Check, Copy, FileVideo2, History, Loader2, Sparkles, Upload } from "lucide-react";
  ...
  const linkMutation = useMutation({
    mutationFn: async ({
      ...
      const update = await getSupabaseBrowserClient()
        ...
        .eq("user_id", user.id);
      if (update.error)
        throw new Error(`Não foi possível alterar o projeto: ${update.error.message}`);
    },
    onSuccess: async () => {
    ...
    onError: (cause) =>
      toast.error(cause instanceof Error ? cause.message : "Não foi possível associar."),
  });
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-7 pb-12">
      ...
      <section className="grid gap-4 md:grid-cols-[1fr_auto]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Sem limite de quantidade de transcrições.
              </p>
            </div>
          </div>
        ...
        <div className="bento-card interactive-card flex min-w-48 items-center gap-4 p-6">
      ...
      <section className="space-y-4">
        ...
        {transcriptionsQuery.isLoading ? (
          ...
          <div className="surface-card p-5 text-destructive">
            {transcriptionsQuery.error.message}
          </div>
          ...
          <div className="space-y-4">
            {transcriptions.map((transcription) => {
              ...
              const project = Array.isArray(transcription.projects)
                ? transcription.projects[0]
                : transcription.projects;
              return (
                <article key={transcription.id} className="bento-card interactive-card p-5 md:p-6">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <Button
                      ...
                      onClick={() => copyText(transcription.id, transcription.transcript || "")}
                    >
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      {analysis.markings.map((mark, index) => (
                        <span
                          ...
                          className="rounded-lg border border-primary/20 bg-primary/[0.06] px-3 py-2 text-xs"
                        >
                  ...
                  {!!analysis.why_it_worked?.length && (
                    <div className="mt-4 rounded-xl border border-emerald-500/20 bg-emerald-500/[0.05] p-4">
                      ...
                      <ul className="mt-2 space-y-1 text-sm text-muted-foreground">
                        {analysis.why_it_worked.map((reason, index) => (
